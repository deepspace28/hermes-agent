...
**Bundled in Hermes** (recommended):

```bash
pip install -r plugins/scraping/hermes-zyte-scraper/requirements.txt
hermes plugins enable scraping/hermes-zyte-scraper
```

Paste API keys on the dashboard **Keys** page (`/env`), in chat when prompted, or via `hermes config set`.

**Standalone plugin** (upstream repo):
...
Set credentials in `~/.hermes/.env` (or use dashboard Keys / in-chat prompts):
