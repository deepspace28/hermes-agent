...
metadata:
  hermes:
    tags: [zyte, scraping, spider, scrapy-cloud]
    category: research
setup:
  help: "Get keys at https://app.zyte.com — enable the hermes-zyte-scraper plugin via `hermes plugins enable scraping/hermes-zyte-scraper`"
  collect_secrets:
    - env_var: ZYTE_API_KEY
      prompt: "Zyte API key"
      provider_url: "https://app.zyte.com/o/zyte-api/api-access"
      secret: true
    - env_var: SCRAPY_CLOUD_API_KEY
      prompt: "Scrapy Cloud API key"
      provider_url: "https://app.zyte.com/o/settings/apikey"
      secret: true
---
...
Paste keys in chat when prompted, on the dashboard **Keys** page (`/env`), or via `hermes config set ZYTE_API_KEY <key>`.
